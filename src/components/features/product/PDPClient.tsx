  ...
  return (
    ...
    <div className="min-h-screen bg-background">
      <div className="container-2m py-8">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs text-text-secondary mb-8 flex-wrap" aria-label="Breadcrumb">
          ...
          <ChevronRight size={12} className={isRtl ? 'rotate-180' : ''} />
          <span className="text-text-primary font-semibold truncate max-w-[200px]">{productName}</span>
        </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-10 xl:gap-16 mb-16">
            ...
            >
              ...
              {product.badge === 'pharmacist-pick' && (
                <div className="absolute top-4 right-4">
                  <span
                    className="badge text-xs bg-brand-primary-soft text-brand-primary border border-brand-primary-soft/30"
                  >
          ...
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-text-primary leading-tight mb-4">
              {productName}
            </h1>
            ...
            {/* Social signals */}
            <div className="flex items-center gap-3 mt-3 mb-5 text-xs text-text-secondary">
            ...
            <div className="flex items-end gap-3 mb-6 pb-6 border-b border-border">
              ...
              {product.originalPrice && (
                <div className="flex flex-col items-start mb-1">
                  <span className="text-sm text-text-muted line-through">{formatEGP(product.originalPrice)}</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-5">
              {/* Qty Stepper */}
              <div
                className="flex items-center rounded-xl overflow-hidden border border-border bg-white"
              >
                <button
                  id="pdp-qty-minus"
                  onClick={() => setQty(Math.max(1, qty - 1))}
                  className="w-11 h-12 flex items-center justify-center text-text-secondary hover:text-brand-primary hover:bg-surface-2 transition-all"
                  aria-label="Decrease quantity"
                >
                  <Minus size={15} />
                </button>
                <span className="w-12 h-12 flex items-center justify-center text-text-primary font-bold text-sm bg-background">
                  {qty}
                </span>
                <button
                  id="pdp-qty-plus"
              ...
              <button
                ...
                className={`btn flex-1 text-sm py-3 transition-all duration-300 font-bold tracking-wide text-white ${added ? 'bg-green-600 border-green-600 text-white' : 'btn-primary btn-shimmer'}`}
            ...
            <div className="grid grid-cols-3 gap-3 mb-7">
              {TRUST_ITEMS.map(({ icon: Icon, labelKey, color }) => (
                <div
                  key={labelKey}
                  className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-center shadow-xs bg-white border border-border"
                >
                  <Icon size={16} style={{ color }} />
                  <span className="text-[10px] text-text-secondary font-semibold leading-tight">{t(labelKey)}</span>
                </div>
              ))}
            </div>
            ...
            <a
              ...
              id="pdp-whatsapp-cta"
              className="flex items-center gap-3 p-4 rounded-xl border border-border bg-surface-2 hover:bg-surface-3 transition-all duration-200 group"
            >
              <MessageCircle size={20} className="text-brand-gold flex-shrink-0" />
              <div>
                <div className="text-sm font-bold text-text-primary">{t('haveQuestions')}</div>
          ...
          <div className="max-w-2xl">
                ...
                <ul className="space-y-2">
                  {product.tags.map((tag) => (
                    <li key={tag} className="flex items-center gap-2">
                      ...
                      <span className="capitalize font-semibold text-text-primary">{tag.replace(/-/g, ' ')}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {activeTab === 'ingredients' && (
              <div className="text-text-secondary text-sm leading-relaxed">
                ...
                <div className="p-4 rounded-xl border border-border bg-surface-2">
                  <p className="text-text-muted text-xs">{t('ingredientsRefer')}</p>
                </div>
              </div>
            )}
            {activeTab === 'usage' && (
            ...
            {activeTab === 'reviews' && (
              <div className="space-y-4">
                {/* Rating Summary */}
                <div
                  className="flex items-center gap-6 p-5 rounded-2xl mb-6 shadow-xs bg-white border border-border"
                >
                  <div className="text-center">
                    <div className="text-5xl font-black text-text-primary">{product.rating}</div>
                    ...
                    <div className="text-xs text-text-secondary font-medium">{t('reviewsCount', { count: product.reviewCount })}</div>
                  </div>
                ...
                {MOCK_REVIEWS.map((review, i) => (
                  <div key={i} className="p-4 rounded-xl border border-border shadow-xs bg-white">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-primary to-brand-primary/80 flex items-center justify-center text-xs font-bold text-white shadow-inner">
                        {review.name[0]}
                      </div>
                      <div>
          ...
          <div>
            <div className="section-header mb-8">
              <div>
                ...
                <h2 className="text-2xl font-black text-text-primary">{t('relatedProducts')}</h2>
              </div>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {related.map((p) => (
                <Link
                  key={p.id}
                  href={`/product/${p.slug}`}
                  className="product-card group block bg-white border border-border rounded-2xl overflow-hidden shadow-xs hover:shadow-sm transition-shadow"
                >
                  ...
                  <div className="p-4">
                    ...
                    <h3 className="text-sm font-semibold text-text-primary leading-tight mb-2 line-clamp-2">{p.name}</h3>
                    <span className="text-sm font-black text-brand-primary">{formatEGP(p.price)}</span>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <AnimatePresence>
        {showSticky && (
          <motion.div
            ...
            animate={{ y: 0 }}
            exit={{ y: 100 }}
            className="fixed bottom-20 left-0 right-0 z-30 p-3 sm:p-4 border-t border-border bg-white/95 backdrop-blur-xl shadow-xs flex items-center justify-between gap-4 sm:bottom-0"
          >
            <div className="hidden sm:flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-bold text-text-primary line-clamp-1 max-w-[250px]">{product.name}</h3>
                <div className="text-sm font-black text-brand-primary">{formatEGP(product.price)}</div>
              </div>
            </div>
            <div className="flex items-center gap-3 w-full sm:w-auto ml-auto">
               <button onClick={handleAddToCart} className="btn btn-primary flex-1 sm:w-64 py-3 h-11 text-sm font-black tracking-wide text-white shadow-xs btn-shimmer">
